...
- Location: tests\homePage.spec.js:29:1
...
Locator: locator('//li[@class=\'selected\']')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('//li[@class=\'selected\']')
...
```ts
  ...
  27 | })
  28 | 
  29 | test ('TC04- Verify One Way Trip option is selected By default', async ({page}) => {
  30 |       const homepage = new HomePage(page);
> 31 |       await expect(homepage.oneWayTrip).toBeVisible();
     |                                         ^ Error: expect(locator).toBeVisible() failed
  32 |       const className = await homepage.isonewayTripSelected();
  33 |       expect(className).toContain("Selected")
  34 | 
  35 | })
  36 | 
  37 | 
